...
It's only necessary to [identify](/docs/product-analytics/identify.md) a user once per session. To prevent sending unnecessary events, check `posthog._isIdentified()` before calling `identify()`:
